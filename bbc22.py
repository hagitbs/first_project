...
import pandas as pd
import numpy as np
df = pd.read_csv('/Users/hagitbenshoshan/Documents/bbc_short.csv') 
...
for _ in range(N_TESTS):
  ...
  df_train, df_val, df_test = np.split(df.sample(frac=1), [int(0.1 * len(df)), int(0.5*len(df))])
...
# cm.stat(summary=True).save_csv("a.csv")
cm.save_csv("test1s.csv")  
cm.save_obj("tstobj")
cm.plot(cmap=plt.cm.Greens, number_label=True, normalized=True )
print('test1 ') 
print(FINAL_CONF_MATRIX) 
cm.plot(cmap=plt.cm.Greens, number_label=True, normalized=True)
plt.savefig("test11.png", format="png")
print (cm) 
cm.save_csv("test1.csv")  
cm.plot(cmap=plt.cm.Greens, number_label=True, normalized=True)
plt.savefig("test111.png", format="png")
...
plt.xticks(np.arange(5)+0.1, labels)
plt.yticks(np.arange(5)+0.1, labels)
plt.show()
plt.savefig("test1111.png", format="png")
#%% Store metrics:
...
# compute AUC_ROC:
auroc = AUROC(num_classes=5)
ovr_AUC = auroc(y_pred_probs, y_test).item()
LabelsOverall = [k for k,v in cm.overall_stat.items() if isinstance(v,(float,int)) ]
LabelsClass = [k for k,v in cm.class_stat.items() if isinstance(v[0],(float,int))]
# Extract metrics from cm object
rawOverall = selectMetrics(cmTest.overall_stat,LabelsOverall) +[ovr_AUC]
rawClass = np.asarray([list(d.values()) for d in selectMetrics(cm.class_stat,LabelsClass)])
...
for _ in range(N_TESTS):
  #Split data into training, additional, validation, and test
  df_train, df_val, df_test = np.split(df.sample(frac=1), [int(0.2 * len(df)), int(0.5*len(df))])
...
print(FINAL_CONF_MATRIX) 
cm.plot(cmap=plt.cm.Greens, number_label=True, normalized=True)
plt.savefig("test12.png", format="png")
print (cm) 
...
for _ in range(N_TESTS):
  #Split data into training, additional, validation, and test
  df_train, df_addl, df_test = np.split(df.sample(frac=1), [int(0.2 * len(df)), int(0.5*len(df))])
...
print(FINAL_CONF_MATRIX) 
cm.plot(cmap=plt.cm.Greens, number_label=True, normalized=True)
plt.savefig("test13.png", format="png")
print (cm) 
# %% [markdown]
...
print(FINAL_CONF_MATRIX) 
cm.plot(cmap=plt.cm.Greens, number_label=True, normalized=True)
plt.savefig("test14.png", format="png")
print (cm) 
# %% [markdown]
